...
// Fetch all selvedge types
function fetchSelvedgeTypes($conn) {
    $stmt = $conn->prepare("SELECT * FROM selvedge_type ORDER BY s1_no ASC");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action'])) {
        if ($input['action'] === 'get_next_code') {
            ...
            $stmt = $conn->prepare("SELECT COALESCE(MAX(s1_no) + 1, 1) AS next_code FROM selvedge_type");
            $stmt->execute();
        ...
        if ($input['action'] === 'add') {
            $selvedge_type = $input['selvedge_type'];

            $stmt = $conn->prepare("INSERT INTO selvedge_type (s1_no, selvedge_type)
                                    VALUES (:s1_no, :selvedge_type)");
            $stmt->bindParam(':s1_no', $input['s1_no']);
            $stmt->bindParam(':selvedge_type', $selvedge_type);
            $stmt->execute();

            echo json_encode(['success' => true, 's1_no' => $input['s1_no'], 'selvedge_type' => $selvedge_type]);
            exit;
        }
        ...
        if ($input['action'] === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM selvedge_type WHERE s1_no IN ($placeholders)");
            $stmt->execute($s1_nos);
...
// Fetch selvedge types for initial display
$selvedgeTypes = fetchSelvedgeTypes($conn);
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <form id="masterForm">
                <table>
                    ...
                    <tbody id="masterTableBody">
                        <?php foreach ($selvedgeTypes as $type): ?>
                        <tr>
                            <td><input type="checkbox" class="rowCheckbox" value="<?= $type['s1_no'] ?>"></td>
                            <td><?= $type['s1_no'] ?></td>
                            <td><?= htmlspecialchars($type['selvedge_type']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        newRow.innerHTML = `
                            ...
                            <td>
                                <div class="input-container">
                                    <input type="text" name="selvedge_type" placeholder="Selvedge Type">
                                </div>
                            </td>
                        ...
                        saveBtn.addEventListener('click', () => {
                            const selvedge_type = newRow.querySelector('input[name="selvedge_type"]').value;
