...
// Fetch all yarn qualities
function fetchYarnQualities($conn) {
    $stmt = $conn->prepare("SELECT * FROM yarn_quality ORDER BY s1_no ASC");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action'])) {
        if ($input['action'] === 'get_next_code') {
            ...
            $stmt = $conn->prepare("SELECT COALESCE(MAX(s1_no) + 1, 1) AS next_code FROM yarn_quality");
            $stmt->execute();
        ...
        if ($input['action'] === 'add') {
            $yarn_quality_id = $input['yarn_quality_id'];
            $yarn_quality = $input['yarn_quality'];

            $stmt = $conn->prepare("INSERT INTO yarn_quality (s1_no, yarn_quality_id, yarn_quality)
                                    VALUES (:s1_no, :yarn_quality_id, :yarn_quality)");
            $stmt->bindParam(':s1_no', $input['s1_no']);
            $stmt->bindParam(':yarn_quality_id', $yarn_quality_id);
            $stmt->bindParam(':yarn_quality', $yarn_quality);
            $stmt->execute();

            echo json_encode(['success' => true, 's1_no' => $input['s1_no'], 'yarn_quality_id' => $yarn_quality_id, 'yarn_quality' => $yarn_quality]);
            exit;
        }
        ...
        if ($input['action'] === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM yarn_quality WHERE s1_no IN ($placeholders)");
            $stmt->execute($s1_nos);
...
// Fetch yarn qualities for initial display
$qualities = fetchYarnQualities($conn);
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <form id="masterForm">
                <table>
                    ...
                    <tbody id="masterTableBody">
                        ...
                        <tr>
                            <td><input type="checkbox" class="rowCheckbox" value="<?= $quality['s1_no'] ?>"></td>
                            <td><?= $quality['s1_no'] ?></td>
                            <td><?= $quality['yarn_quality_id'] ?></td>
                            <td><?= $quality['yarn_quality'] ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        newRow.innerHTML = `
                            ...
                            <td>
                                <select name="yarn_quality_id">
                                    <option value="K">K</option>
                                    <option value="CC">CC</option>
                                </select>
                            </td>
                            <td>
                                <select name="yarn_quality">
                                    <option value="Karded">Karded</option>
                        ...
                        saveBtn.addEventListener('click', () => {
                            const yarn_quality_id = newRow.querySelector('select[name="yarn_quality_id"]').value;
                            const yarn_quality = newRow.querySelector('select[name="yarn_quality"]').value;
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    newRow.innerHTML = `
                                        ...
                                        <td>${data.s1_no}</td>
                                        <td>${data.yarn_quality_id}</td>
                                        <td>${data.yarn_quality}</td>
                                    `;
                                } else {
