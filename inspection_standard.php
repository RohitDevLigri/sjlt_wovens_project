...
// Fetch all inspection standards
function fetchInspectionStandards($conn) {
    $stmt = $conn->prepare("SELECT * FROM inspection_standard ORDER BY s1_no ASC");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action'])) {
        if ($input['action'] === 'get_next_code') {
            ...
            $stmt = $conn->prepare("SELECT COALESCE(MAX(s1_no) + 1, 1) AS next_code FROM inspection_standard");
            $stmt->execute();
        ...
        if ($input['action'] === 'add') {
            $inspection_standard = $input['inspection_standard'];

            $stmt = $conn->prepare("INSERT INTO inspection_standard (s1_no, inspection_standard)
                                    VALUES (:s1_no, :inspection_standard)");
            $stmt->bindParam(':s1_no', $input['s1_no']);
            $stmt->bindParam(':inspection_standard', $inspection_standard);
            $stmt->execute();

            echo json_encode(['success' => true, 's1_no' => $input['s1_no'], 'inspection_standard' => $inspection_standard]);
            exit;
        }
        ...
        if ($input['action'] === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM inspection_standard WHERE s1_no IN ($placeholders)");
            $stmt->execute($s1_nos);
...
// Fetch inspection standards for initial display
$inspectionStandards = fetchInspectionStandards($conn);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Inspection Standard</title>
            ...
            <form id="masterForm">
                <table>
                    ...
                    <tbody id="masterTableBody">
                        <?php foreach ($inspectionStandards as $inspection): ?>
                        <tr>
                            <td><input type="checkbox" class="rowCheckbox" value="<?= $inspection['s1_no'] ?>"></td>
                            <td><?= $inspection['s1_no'] ?></td>
                            <td><?= htmlspecialchars($inspection['inspection_standard']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        newRow.innerHTML = `
                            ...
                            <td>
                                <input type="text" name="inspection_standard" placeholder="Inspection Standard">
                            </td>
                        ...
                        saveBtn.addEventListener('click', () => {
                            const inspection_standard = newRow.querySelector('input[name="inspection_standard"]').value;
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    newRow.innerHTML = `
                                        ...
                                        <td>${data.s1_no}</td>
                                        <td>${data.inspection_standard}</td>
                                    `;
                                } else {
