...
try {
    $stmt = $conn->query("SELECT MAX(s1_no) AS last_s1_no FROM agent_details");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $gstin = $_POST['gstin'];
        $pan_number = !empty($_POST['pan_number']) ? $_POST['pan_number'] : null;
        $address = $address_street . ', ' . $address_city . ', ' . $address_postal;
        $sql = "INSERT INTO agent_details (
            s1_no, agent_code, agent_name, invoice_address, contact_no, contact_person,
            address, city_pin, state, country, category,
        ...
        <div class="container">
            ...
            <form method="POST" action="">
                <div class="buttons-container">
                    <button type="button" onclick="window.location.href='agent_details_summary.php'">Back</button>
