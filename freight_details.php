...
// Fetch all freight details
function fetchFreightDetails($conn) {
    $stmt = $conn->prepare("SELECT * FROM freight_details ORDER BY s1_no ASC");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action'])) {
        if ($input['action'] === 'get_next_code') {
            ...
            $stmt = $conn->prepare("SELECT COALESCE(MAX(s1_no) + 1, 1) AS next_code FROM freight_details");
            $stmt->execute();
        ...
        if ($input['action'] === 'add') {
            ...
            $stmt = $conn->prepare("INSERT INTO freight_details (s1_no, freight)
                                    VALUES (:s1_no, :freight)");
            $stmt->bindParam(':s1_no', $input['s1_no']);
            $stmt->bindParam(':freight', $freight);
            $stmt->execute();

            echo json_encode(['success' => true, 's1_no' => $input['s1_no'], 'freight' => $freight]);
            exit;
        }
        ...
        if ($input['action'] === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM freight_details WHERE s1_no IN ($placeholders)");
            $stmt->execute($s1_nos);
...
// Fetch freight details for initial display
$freightDetails = fetchFreightDetails($conn);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <main>
        <div class="container">
            ...
            <button type="button" onclick="window.location.href='freight_details.php'" class="saveBtn">Back</button>
            <form id="masterForm">
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody id="masterTableBody">
                        <?php foreach ($freightDetails as $freight): ?>
                        <tr>
                            <td><input type="checkbox" class="rowCheckbox" value="<?= $freight['s1_no'] ?>"></td>
                            <td><?= $freight['s1_no'] ?></td>
                            <td><?= htmlspecialchars($freight['freight']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        saveBtn.addEventListener('click', () => {
                            const freight = newRow.querySelector('select[name="freight"]').value;
