...
function fetchDeliveryTerms($conn) {
    $stmt = $conn->prepare("SELECT * FROM delivery_terms ORDER BY s1_no ASC");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action'])) {
        if ($input['action'] === 'get_next_code') {
            ...
            $stmt = $conn->prepare("SELECT COALESCE(MAX(s1_no) + 1, 1) AS next_code FROM delivery_terms");
            $stmt->execute();
        ...
        if ($input['action'] === 'add') {
            ...
            $stmt = $conn->prepare("INSERT INTO delivery_terms (s1_no, delivery_terms)
                                    VALUES (:s1_no, :delivery_terms)");
            $stmt->bindParam(':s1_no', $input['s1_no']);
            $stmt->bindParam(':delivery_terms', $delivery_terms);
            $stmt->execute();

            echo json_encode(['success' => true, 's1_no' => $input['s1_no'], 'delivery_terms' => $delivery_terms]);
            exit;
        }
        ...
        if ($input['action'] === 'delete') {
            ...
            $stmt = $conn->prepare("DELETE FROM delivery_terms WHERE s1_no IN ($placeholders)");
            $stmt->execute($s1_nos);
...
// Fetch delivery terms for initial display
$deliveryTerms = fetchDeliveryTerms($conn);
?>
<!DOCTYPE html>
<html lang="en">
<head>
            ...
            <form id="masterForm">
                <table>
                    ...
                    <tbody id="masterTableBody">
                        <?php foreach ($deliveryTerms as $term): ?>
                        <tr>
                            <td><input type="checkbox" class="rowCheckbox" value="<?= $term['s1_no'] ?>"></td>
                            <td><?= $term['s1_no'] ?></td>
                            <td><?= htmlspecialchars($term['delivery_terms']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        newRow.innerHTML = `
                            ...
                            <td>
                                <input type="text" name="delivery_terms" placeholder="Delivery Terms">
                            </td>
                        ...
                        saveBtn.addEventListener('click', () => {
                            const delivery_terms = newRow.querySelector('input[name="delivery_terms"]').value;
